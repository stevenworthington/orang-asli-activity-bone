###############################################################################
# Shared helper functions for these analyses: package loading
...
# through renv (via the pak backend), never through this wrapper.
load_pkgs <- function(...) {
  pkgs <- c(...)
...
#
# Kept in the helper set even though curvature is not part of the published
# figure convention (those show AERF + AMEF only; flatness /
...
# ---- Panel + stack builders ----
#
# Each "cell" is a vertical stack of an AERF panel
...
make_aerf_panel <- function(spec, simul_bands_aerf, lin_proj,
  ...
  # Two-line y-axis label, so the title
...
# slices (color-coded by age) for a single spec. Used by the supplement figure
# on whether PA effects vary by age. Uncertainty is omitted at the panel level (4
...
# busier than the medians-only make_age_cond_amef_panel, and the one used for
# the published supp-fig-7-age-conditional-bands.pdf.
...
# ---- aerf_amef_plot: 3-panel working figure (AERF / AMEF / curvature) ----
#
# The per-spec diagnostic figure each Tier-2 script saves to
# outputs/figures/working/<stem>/. It adds the curvature panel, which the
# published figures omit -- those use the AERF + AMEF 2-panel stack, via
# make_aerf_panel / make_amef_panel / stack_subfig above.
...
# threshold, so a caller can sweep across "what counts as negligible."
