...
# adjustment-set covariates distributed as observed in the cohort").
# The `datagrid(model = fit)` default (`grid_type = "mean_or_mode"`) would
# instead give AERFs at typical covariate values, a different estimand.
...
# Used for age-conditional AMEF reporting (are PA effects largest before peak
# bone mass?). Returns one long-format tibble with all age slices stacked,
# plus an `age_years` column.
#
...
amef_at_age <- function(fit, spec, age_value, dat_raw) {
  ...
  # the population-average AMEF in amef_draws().
  dg_args <- list(model = fit, grid_type = "counterfactual",
                  age_years = age_value)
  dg_args[[spec$exposure]] <- grid
  new_grid <- do.call(marginaleffects::datagrid, dg_args)
