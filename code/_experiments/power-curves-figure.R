...
# PA->bone panels (green) are well-powered; industrialization panels (red,
# community level) are not.
# Output: outputs/figures/final/supp-fig-5-power-curves.{pdf,png}
#
# Run: Rscript code/_experiments/power-curves-figure.R
###############################################################################
...
ur <- ub |>
  ...
  transmute(key, group = "Industrialization → outcome (community level)",
            effect_sd, power, resolution_sd)
...
cols <- c("Physical activity → bone (individual level)"    = "#3F7E4E",
          "Industrialization → outcome (community level)"     = "#A85D52")
...
p <- ggplot(dat, aes(effect_sd, power, color = group)) +
  ...
  labs(x = "True effect size (outcome SD units, over the meaningful contrast: 5,000 steps / 10 mg ENMO, or across the industrialization gradient)",
       y = "Power", color = NULL,
       title = "A priori power curves (frequentist proxy, validated against the reported Bayesian estimator; N = 2,000 per point)",
...
out_dir <- here("outputs", "figures", "final")
