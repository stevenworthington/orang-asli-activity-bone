...
# CVs from Methods; qUS in-study test-retest SEM) to give two reference scales per outcome.
dat <- read.csv(here::here("data", "processed", "orang-asli-pa-bone-data.csv"))
...
for (nm in names(cols)) {
  ...
  cat(sprintf("%-22s n=%4d | mean=%11.3f | median=%11.3f | SD=%11.3f | CV=%5.1f%%\n",
              nm, length(v), mean(v), stats::median(v), stats::sd(v), 100 * stats::sd(v) / mean(v)))
}
