###############################################################################
# Walking-experiment reverse-ROPE analysis (reported in the supplement).
#
# Analyzes the acute walking experiment. A non-significant rank test cannot
# separate a genuine null from a power artifact, so the question is answered
# here instead with the project's reverse-ROPE
...
# Used only to place the resolvability floor, the experiment's own CV not
# being available. The resolvability floor follows the supplement's
# convention: total analytical CV (intra ⊕ inter, in quadrature) at the sample
# median.
INTRA_CV <- 0.020
INTER_CV <- 0.028
TOTAL_CV <- sqrt(INTRA_CV^2 + INTER_CV^2)   # 3.44%
