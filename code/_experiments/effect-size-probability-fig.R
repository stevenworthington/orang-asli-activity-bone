...
#   - NULL (PA -> bone):    effect = |proj slope x fixed contrast|;   report U95 (<=, rule out larger).
# The PA-null contrasts are fixed a priori: daily steps = 5,000 steps (5 units
# of ad_steps_1k); mean daily ENMO = 10 mg. Each panel also marks the
# measurement-noise floor for its OUTCOME.
...
specs <- list(
  list(key="sos.urb",    ex="industrial_index",       lab="SOS ~ industrialization",   unit="m/s",   type="detected", out="sos",   village_level=TRUE, tag="community-level"),
  list(key="enmo.urb",   ex="industrial_index",       lab="ENMO ~ industrialization",  unit="mg",    type="detected", out="enmo",  village_level=TRUE, tag="community-level"),
  list(key="steps.urb",  ex="industrial_index",       lab="Steps ~ industrialization", unit="steps", type="detected", out="steps", village_level=TRUE, tag="community-level"),
...
# Compact panel size with tightened margins and k-formatted thousands ticks so
...
for (i in seq_along(specs)) {
  ...
  if (isTRUE(s$village_level)) {                                              # village-level two-stage
  ...
  if (s$type == "hump") {
    ...
    eff <- abs(as.numeric((M %*% xc) / sum(xc^2)) * s$w)           # |slope x fixed contrast|
