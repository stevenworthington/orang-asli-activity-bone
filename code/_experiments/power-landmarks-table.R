###############################################################################
# Power-landmarks table — companion to Supp Fig 5 (the a priori power curves).
...
# the two sweep scripts include the landmark effect sizes as explicit grid points,
...
#   stdout: a GitHub-markdown table for pasting into the supplement.
#
# Run: Rscript code/_experiments/power-landmarks-table.R
###############################################################################

suppressMessages(library(here))

pb <- read.csv(here("outputs", "_experiments", "power-curves", "pa-bone-power.csv"))
ub <- read.csv(here("outputs", "_experiments", "power-curves", "industrialization-power-onestage.csv"))

# community counts -- the level of inference for these analyses, and the same
# real design the power simulation uses.
...
# ---- industrialization -> outcome (community level) --------------------------
...
ur <- do.call(rbind, lapply(names(ub_lab), function(k) {
  ...
  data.frame(
    key = k, label = ub_lab[[k]], level = "Industrialization → outcome (community level)",
...
out_csv <- file.path(out_dir, "power-landmarks.csv")
...
fmtN  <- function(r) if (grepl("individual", r$level)) sprintf("%d", r$n) else sprintf("%d communities", r$n_villages)
