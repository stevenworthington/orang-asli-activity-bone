###############################################################################
# Industrialization -> daily step count (rescaled to /1000). t2(age,
# industrial_index) tensor smooth + minimal adjustment {sex}. Reads formula
# / priors / MCMC config from
# the registry in `code/_startup/specifications.R`.
###############################################################################
...
# Remove the global dat_local before postestimation. Not strictly
# needed here (this spec's only factor, sex, is always fully represented), but
# kept for consistency with the other Tier-2 scripts where it is load-bearing.
rm(dat_local)
