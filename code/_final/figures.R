...
# assembles publication-ready PDFs in the project's figure convention: each
# cell is an AERF panel stacked over an AMEF panel sharing the exposure
# x-axis, with nested grey simul-bands (5/25/50/75/95%) + red-dashed
# linear-projection overlay.
#
# Outputs:
...
#
# Population-average summaries only -- the t2(age, exposure) tensor smooth is
# marginalized over the cohort's observed covariate distribution via a
# counterfactual datagrid (see aerf_draws() / amef_draws() in
...
# Visual layout (column-major panel letters):
#
...
fig_pa_bone <- patchwork::wrap_plots(set_1_panels, ncol = 2)
ggsave(fig_pa_bone, file = file.path(final_dir, "fig-4-pa-bone.pdf"),
...
# 1 row x 3 cols landscape, ordered tibial SOS, ENMO, daily steps.
...
fig_urb <- patchwork::wrap_plots(set_2_panels, ncol = 3)
# Sized so each panel matches the PA -> bone figure's panel size (that one is
# 5.4 x 8.8 for 2 cols x 3 rows -> ~2.7" wide x ~2.93" tall per cell). This
# one has 3 cols x 1 row, so 3 x 2.7" = 8.1" wide and 2.93" tall.
ggsave(fig_urb, file = file.path(final_dir, "fig-3-urb.pdf"),
...
# Mirrors the PA -> bone figure's 3x2 grid layout, but uses the *-conf spec keys. The
...
# regression covariates). Confounder-DAG
# fits use the same prior structure as the mediator-DAG fits plus the two
# body-comp coefficients. Per-column x-range unified the same way there; only the bottom row carries the x-axis text.
...
# ---- Age-conditional AMEF supplement figure ----
#
# Mirrors the 3x2 Fig 4 grid (mediator-DAG PA -> bone) but each cell shows
# four median AMEF curves color-coded by age (25 / 35 / 50 / 65). It addresses
# the possibility that PA effects on bone are largest before peak bone mass
...
# Age-conditional AMEFs WITH 95% HPDI bands -- the sole age-conditional
# figure. Each age slice gets a
